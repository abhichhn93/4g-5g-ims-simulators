...
// TCP socket wrapper — RAII, length-framed messages.
// Shared pattern across the 4G/5G/IMS/CBRS simulators in this repo.
// Real CBRS uses TLS + HTTPS; this simulator uses plain TCP + JSON frames
// to keep dependencies at zero and the learning curve low.
class Socket {
public:
    Socket() : fd_(-1) {}
    ...
    static Socket createServer(const char* ip, uint16_t port, int backlog = 5) {
        ...
        if (::bind(fd, reinterpret_cast<sockaddr*>(&a), sizeof(a)) < 0) {
            ::close(fd);
            throw std::runtime_error("bind() failed port " + std::to_string(port));
        }
    ...
    // Accepts hostname OR IP — resolves via getaddrinfo so Docker/K8s
    // service names work without any code change.
    ...
    // Wire framing: [4-byte LE length][JSON payload]
    bool sendFrame(const std::vector<uint8_t>& frame) const {
        return sendAll(frame.data(), uint32_t(frame.size()));
    }
    bool recvFrame(std::vector<uint8_t>& payload) const {
        uint32_t plen = 0;
        if (!recvAll(&plen, 4)) return false;
        if (plen == 0 || plen > 65536) return false;
        payload.resize(plen);
        return recvAll(payload.data(), plen);
    }
